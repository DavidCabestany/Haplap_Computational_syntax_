...
class HMM:
    ...
    def viterbi(self, O):
        ...
        for current_timestep in range(1, len(O)):
            for currentstate in range(0, len(self.Q)):
                
                maxscore = np.max(trellis[:, current_timestep-1] * self.A[:,currentstate] )
                trellis[currentstate, current_timestep] = maxscore * self.B[O[current_timestep]-1][currentstate]

            backpointer[current_timestep] = np.argmax(trellis[:, current_timestep])
